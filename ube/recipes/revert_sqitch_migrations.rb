# Recipe name: revert_sqitch_migrations
#
...
if change.to_s.strip.length == 0
  command = "sqitch --engine #{engine} --registry #{registry} revert #{dburi}"
else
  command = "sqitch --engine #{engine} --registry #{registry} revert #{dburi} --to #{change}"
end
...
Chef::Log.info("Completed reverting sqitch migrations")
